...
#Give a visualized map showing multiple widgets that filter job posting distribution over US.
#get_geo_world() function given is referenced from webpage https://groups.google.com/a/continuum.io/forum/#!topic/bokeh/cJ7tzsm1aLc
###############################################
...
#original world-geo json file given by Johan Sundström https://github.com/johan/world.geo.json
#url above provided by Dennis O'Brien and following codes come from Jean Felder
#   on Google Groups 'Bokeh User mailing list' https://groups.google.com/a/continuum.io/forum/#!topic/bokeh/cJ7tzsm1aLc
#thanks to their extraodinary work!
#parmas:
...
color_bar = ColorBar(
    color_mapper = LinearColorMapper(palette=palette1,
        low = 0,#min(source.data['count']),
        high = 1),#max(source.data['count'])),
    ticker=FixedTicker(ticks=[0,0.2,0.4,0.6,0.8,1]),#BasicTicker(),
...
select_experience = Select(
    title="Select working experience",
    value='All',
